...
# deployment branch this clone holds behind its counterpart.  The third
# discards nothing, its caller having asked resolve_branch first and moving
# only on behind.  It refuses the checked-out branch, because git's own
# `branch -f` refuses there and a ref that disagreed with the working tree
# beside it would be worse than a refusal.
sub set_branch_ref {
