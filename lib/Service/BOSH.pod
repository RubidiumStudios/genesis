...
=head2 command
  ...
  my $path = Service::BOSH->command();
  my $path = Service::BOSH->command($min_version);
...
B<Returns:> The full path to the selected BOSH CLI executable as a string.
...
boolean return. See the C<execute-scalar-return> entry in L</KNOWN ISSUES>.
...
The following defects are documented here so
that callers are aware of the current behaviour.

=over 4

=item execute-scalar-return (low)
