...
Dies (via C<bug()>) with C<"BOSH returned multiple entries for $_-E<gt>{label} - Genesis doesn't know how to process this">
...
C<download-configs-garbled-message> in L</KNOWN ISSUES>: the error message
string uses C<$_-E<gt>label> without braces, producing a garbled label like
C<"HASH(0x...)-E<gt>label">; the intended form is C<$_-E<gt>{label}>).
...
The following defects are documented here to
prevent callers from relying on the current (incorrect) behavior.

=over 4

=item deployment-setter-guard (high)
...
=item errand-discards-exit-code (medium)
...
=item download-configs-garbled-message (medium)

C<download_configs()>: C<$_-E<gt>label> inside a double-quoted string
interpolates C<$_> as the stringified hashref (e.g. C<HASH(0x...)>)
followed by the literal text C<-E<gt>label>, producing a garbled error
message. The correct form is C<$_-E<gt>{label}>.

=item upload-config-discards-return (medium)
