...
C<pve>, C<vsphere>, C<virtualbox>, C<warden>.
...
Dies with C<"Unable to reach bosh.io to list stemcells for #C{%s} on the #C{%s} CPI (HTTP status: %s)."> --
followed by advice to pin an already-uploaded stemcell version or retry --
when the bosh.io request returns anything other than success. The
substitutions are the OS slug, the CPI, and the HTTP status, or C<none> when
the request produced no status at all.

The status is checked before the body is parsed, so an unreachable bosh.io
reports as unreachable rather than as a JSON parse failure.
...
the filtered type (see L</KNOWN ISSUES>, type-filter-key-selection). The C<type> filter is applied
...
See the note above regarding type-filter-key-selection.
...
Returns true if the stemcell's type matches C<$type>.
...
The type string to compare: C<'light'> or C<'regular'>.

=back

B<Returns:> True if the stemcell's type equals C<$type>, false otherwise.
...
B<Returns:> True if the stemcell type is C<'light'>, false otherwise.
...
B<Returns:> True if the stemcell type is C<'regular'>, false otherwise.

B<Examples:>

  if ($sc->is_full()) {
    print "Full (regular) stemcell\n";
  }
  # Returns: 1 for a regular stemcell
...
B<Returns:> True if the stemcell URL does not start with C<http://> or
C<https://>, false otherwise.

B<Examples:>

  if ($sc->is_local()) {
    print "Local stemcell file\n";
  }
  # Returns: 1 when url is a file path rather than an http/https URL

=head2 download

  $sc->download($file);

B<Not implemented.> Always dies with
C<"Downloading stemcell from %s is not implemented yet">, where C<%s> is the
stemcell's URL. The C<$file> parameter and the C<return 1> statement that
follows the error are unreachable dead code.
...
The intended local file path. Currently unused; the method always dies before
reaching any file-writing logic.

=back

B<Returns:> Never returns normally.

B<Errors:>

Always dies with C<"Downloading stemcell from %s is not implemented yet">.
C<%s> is the stemcell's URL.

B<Examples:>

  eval { $sc->download('/tmp/stemcell.tgz') };
  # Returns: never -- always dies with "Downloading stemcell from ... is not implemented yet"
...
The following defects are documented here so that callers are aware of the
current behaviour.

=over 4

=item type-filter-key-selection (low)
