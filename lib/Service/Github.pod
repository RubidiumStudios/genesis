...
without merging, and the same C<review> hashref C<open_prs> attaches. That is
what tells a merged proposal from a rejected one: the supersedes list names
the rejected attempts and quotes whoever asked for the change, while the
recovery reads a merged one's body.
