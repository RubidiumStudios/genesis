...
Compares two ranges span-by-span using each span's start and end addresses. Returns -1, 0, or 1. The C<$swap> parameter is used internally by Perl's operator overloading and should not be passed directly.
...
Checks if two ranges are equal based on their string representations. The C<$swap> parameter is used internally by Perl's operator overloading and should not be passed directly.
...
Checks if two ranges have the same size. Returns true if their sizes are equal, false otherwise. The C<$swap> parameter is used internally by Perl's operator overloading and should not be passed directly.
...
Compares two ranges numerically based on their sizes. Returns -1, 0, or 1. The C<$swap> parameter is used internally by Perl's operator overloading and should not be passed directly.
...
Internal method called by the overloaded C<-> operator. Dies if C<$other> is an integer, since adding/subtracting integers on ranges is undefined. When C<$other> is an IPv4 object, subtracts it from the range (or vice versa if C<$swap> is true). The C<$swap> parameter is used internally by Perl's operator overloading and should not be passed directly.
