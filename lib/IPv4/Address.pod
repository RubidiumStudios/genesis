...
Compares two IPv4 addresses. Returns -1, 0, or 1 if the first address is less than, equal to, or greater than the second address, respectively. Comparison is based on the integer value of the addresses, with size as a tiebreaker for mixed types (e.g., comparing an address to a span). The C<$swap> parameter is set automatically by Perl when the object appears on the right side of an operator (e.g., C<$string cmp $address>) and should not be passed directly.
...
Compares two IPv4 objects by size. Returns -1, 0, or 1 if the size of the first object is less than, equal to, or greater than the size of the second, respectively. Can also compare the size to an integer. While the size of an address is always 1, this method is included for polymorphic consistency with C<IPv4::Span> and C<IPv4::Range>. The C<$swap> parameter is set automatically by Perl when the object appears on the right side of an operator and should not be passed directly.
