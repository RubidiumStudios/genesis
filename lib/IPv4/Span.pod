...
Returns the network address (the first address in the span). Similar to NetAddr::IP's network() method.
...
Returns the broadcast address (the last address in the span). Similar to NetAddr::IP's broadcast() method.
...
Returns the first usable host address in the span (typically network address + 1). 
...
Returns the last usable host address in the span (typically broadcast address - 1).
...
Returns a new range containing only the usable host addresses (excludes network and broadcast addresses if span size > 1).
Similar to iterating through usable addresses with NetAddr::IP's nth() method.
  ...
  my $small = IPv4::Span->new('192.168.1.1');
  my $empty = $small->usable_hosts();
  print $empty->size(); # prints '0' (no usable hosts in a single-IP span)

=head2 nth($index [, $include_reserved])

Returns the IPv4::Address at the specified index within the span, or undef if the
index is out of range. This method is similar to the nth() method in NetAddr::IP.

If called without the optional $include_reserved parameter (or with a false value),
it will skip the network and broadcast addresses in standard networks (/30 or larger).
In this case, index 0 refers to the first usable host address (typically network+1).

If called with $include_reserved set to a true value, it will include network and
broadcast addresses, and index 0 will refer to the network address.

Negative indices count from the end of the span (with -1 representing the last address).
If $include_reserved is false, this will be the last usable host address rather than
