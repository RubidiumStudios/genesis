...
from C<STDIN> and write their questions, menus, and banners through
L</__prompt_print>, which goes to C<STDERR> and to the controlling terminal.
Nothing a prompt writes goes to C<STDOUT>, so a caller can pipe the command's
output into a parser without a menu landing in it.
...
=head2 prompt_for_line
    ...
    my $value = prompt_for_line($prompt, $label, $default, $validation, $err_msg, $hide_response);
...
=item C<$hide_response>

Optional. When true B<and> the process is attached to a controlling
terminal, terminal echo is disabled around the C<STDIN> read so the
typed value is not displayed. See L</prompt_for_password> for the common
wrapper that uses this flag.
...
=head2 prompt_for_password

    my $secret = prompt_for_password($label);

Reads a single line of input from C<STDIN> with terminal echo disabled, so
the typed value is not displayed. Intended for passwords, tokens, AppRole
secret-ids, GitHub PATs, and other sensitive credentials.

Calls L</__prompt_for_line> with C<allow_blank> set so an EOF or empty
response returns the empty string rather than looping for retry. Hidden
input only takes effect when attached to a controlling terminal; in non-TTY
environments (tests, piped input) the read happens normally so test
...
Short label shown next to the C<E<gt>> prompt (e.g. C<"Password">,
C<"GitHub Personal Access Token">).

=back

B<Returns:> The line entered with the trailing newline stripped. Returns
the empty string on EOF or when the user presses Enter without typing
anything.

B<Examples:>

    my $token = prompt_for_password("Vault Token");
    # Returns: the typed token, no echo to the screen
...
=head2 __bail_on_eof
    ...
    __bail_on_eof($prompt)

Stops with a clear message when the last read in L</__prompt_for_line> hit
EOF on C<STDIN>.  The prompt loops that treat a blank answer as "ask again"
(L</prompt_for_boolean> with no default, and L</prompt_for_choices> and
L</prompt_for_list> below their minimum) call this before asking again, so
an exhausted standard input ends the command instead of spinning through
the question forever.  Does nothing when the last read returned a line.

B<Errors:>

=over 4

=item C<"Cannot read an answer to \"%s\": standard input is exhausted and no terminal is attached.  Run this command in a terminal, or use --yes where the command supports it.">

Raised (via C<bail()>) when the last read hit EOF.

=back

B<Examples:>

    __bail_on_eof("Continue? [y|n]");
    # stops when STDIN is exhausted; returns otherwise

=head2 __prompt_needs_terminal_copy

    __prompt_needs_terminal_copy()

True when a prompt written to C<STDERR> would not be seen: C<STDIN> is a
terminal, so someone is there to answer, but C<STDERR> is a pipe or a file,
so the question written there never reaches them.  L</__prompt_print>
consults this before writing its copy to the controlling terminal.

B<Returns:> C<1> when the terminal copy is needed, C<0> otherwise.

B<Examples:>

    __prompt_needs_terminal_copy();
    # 1 under `genesis ... 2>&1 | tee log`, run from a terminal
    # 0 when STDERR is the terminal, and 0 when STDIN is not

=head2 __open_controlling_terminal

    __open_controlling_terminal()

Opens C</dev/tty> for writing and returns the handle, or C<undef> when the
process has no controlling terminal.

B<Returns:> A write handle on the controlling terminal, or C<undef>.

B<Examples:>

    my $tty = __open_controlling_terminal();
    # a filehandle, or undef when there is no controlling terminal

=head2 __prompt_print

    __prompt_print($text)

Writes prompt text to C<STDERR>, and also to the controlling terminal
(C</dev/tty>) when C<STDIN> is a terminal but C<STDERR> is not.  A command
run as C<genesis ... 2E<gt>&1 | tee log | grep ...> sends its prompts into
the pipe, where a filter can drop them, while the read that follows waits on
the terminal for an answer to a question nobody saw; writing the prompt to
the terminal as well keeps it visible.

Every prompt and every banner in this module goes through here, so nothing a
prompt writes lands on C<STDOUT>.  That keeps C<--json> output, and anything
else a caller pipes into a parser, free of menus and questions.

When L</__prompt_needs_terminal_copy> is true and
L</__open_controlling_terminal> returns C<undef>, there is nowhere left to
show the question, so the command stops here rather than waiting on an
...
=item C<"Cannot reach a terminal to ask a question, and standard error is redirected, so the question cannot be shown.  Run this command with its standard error attached to a terminal, or use --yes where the command supports it.">

Raised (via C<bail()>) when the terminal copy is needed and C</dev/tty>
cannot be opened.
...
B<Examples:>
    ...
    __prompt_print("Continue? [y|n] > ");
    # Appears on STDERR, and on the terminal when STDERR is redirected

=head2 __prompt_for_line

    __prompt_for_line($prompt, $validation, $err_msg, $default, $allow_blank, $hide_response)
...
When C<$hide_response> is true B<and> the process is attached to a controlling
terminal (per L<Genesis::Term/in_controlling_terminal>), terminal echo is
disabled around the C<STDIN> read so the typed value is not displayed. In
non-TTY environments (tests, piped input) the read happens normally so test
harnesses driving the function via a piped C<STDIN> are unaffected.

EOF on C<STDIN> means nobody is going to answer.  A defined C<$default> is
taken as the answer, and C<$allow_blank> returns C<"">; otherwise the prompt
stops with a message that standard input is exhausted and no terminal is
attached, rather than spinning the retry loop or waiting forever.  The
prompt text itself goes through L</__prompt_print>, so it reaches the
terminal even when C<STDERR> is redirected.

B<Errors:>

=over 4

=item C<"Error compiling param regex: $!">

Dies if the C<$validation> string is a regex-format specifier and the
pattern fails to compile.

=item C<"Cannot read an answer to \"%s\": standard input is exhausted and no terminal is attached.  Run this command in a terminal, or use --yes where the command supports it.">

Raised (via C<bail()>) on EOF when there is no default and a blank answer
is not allowed.
...
B<Examples:>
    ...
    # Hidden input (used by prompt_for_password)
    my $secret = __prompt_for_line("Password", undef, undef, undef, 1, 1);
    # Returns: user input; nothing is echoed to the terminal during typing
