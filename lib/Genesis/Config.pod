...
Keys that exist only in the I<env> or I<default> layers (populated by
C<validate()>) remain accessible via C<has()> and C<get()> after C<clear()> -
those layers are not modified.
...
B<Examples:>
    ...
    $cfg->clear('temp_key');
    print $cfg->has('temp_key') ? "yes" : "no";  # Returns: no (no default layer value)
