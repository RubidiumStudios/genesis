...
It owns the block's defaults outright: whatever a previous call filled
under C<$path> is dropped before this schema fills its own, so a block
validated against one schema and then another does not carry the first
one's defaults into the second one's walk and have them refused there as
