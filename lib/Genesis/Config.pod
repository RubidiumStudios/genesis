...
=head2 schema_has
    ...
    my $known = $cfg->schema_has('ci.provider.type');
...
=head2 _schema_for_key
    ...
    my $spec = $cfg->_schema_for_key('ci.provider.type');
...
clearing C<ci.enabled> leaves C<ci =E<gt> {}> in whichever source
structure did not hold the key. That empty hash is not harmless: the
merge is by priority rather than depth, so an empty C<ci> in the higher
priority source wins outright and takes the key's siblings with it --
C<ci.provider> would vanish from what C<save()> writes, without ever
having been touched.
...
B<Examples:>
    ...
    # after clearing ci.enabled from a config also holding ci.provider
    $cfg->_prune_empty_parents($cfg->{set_values}, 'ci.enabled');
    # ci is left in place, still carrying provider
...
C<set('ci.enabled', 'false')> would store a true boolean, which validates
...
B<Examples:>
    ...
    $cfg->_coerce_to_schema_type('ci.enabled', 'false');   # JSON::PP::false
