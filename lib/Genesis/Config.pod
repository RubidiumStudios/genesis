...
Removing a key whose ancestor holds a scalar does nothing, rather than
failing. A schema may declare a scalar default at a top-level key, and
nothing is stored beneath a scalar, so a path under one names no value in
the I<default> store and there is nothing there to remove.
