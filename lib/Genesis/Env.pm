...
sub _manifest_secret_credhub_conflicts {
	...
	# Silent no-match would hide real conflicts and let fix-secrets
	# regenerate a divergent copy; assert the prefix instead.
	my $credhub_prefix = $self->credhub->base;
	bail("credhub base is unset; cannot check for manifest-secret conflicts")
		unless defined($credhub_prefix) && length($credhub_prefix);
	$credhub_prefix .= '/' unless $credhub_prefix =~ m{/$};
