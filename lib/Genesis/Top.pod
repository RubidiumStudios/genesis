...
B<Examples:>
    ...
    my $bool = $top->ci_configured();
    # returns false in a repo with pipeline.enabled but no provider
...
the deployment type. A pipeline with no provider type is a manual one that
an operator drives by hand, so an absent block means manual rather than no
pipeline at all.
