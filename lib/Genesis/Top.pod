...
github-actions one. The two repository-wide keys are read off the validated
configuration, and the two per-environment keys are read out of each
environment's merged C<genesis.pipeline> block, so every gate that fires is
gathered before the refusal is raised.
...
B<Examples:>
    ...
    $top->_validate_capability_gates;
    # exits CONFIG (78) where pipeline.provider.type is concourse and an
    # environment sets genesis.pipeline.manual under a provider that
    # declares optional_git_triggers false
