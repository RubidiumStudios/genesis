...
resolved, which is where the derivations and their refusals are raised, and
then every environment the deployment root holds has its
C<genesis.pipeline> block checked, read merged through
L</_merged_env_params>. The refusals themselves belong to
L</_source_control> and to L</_validate_env_pipeline_block>.
