...
B<Examples:>
    ...
    Genesis::Top->register_config_section('ci', \&ci_handler);
    # returns the handler; the 'ci' section is now handled by it
...
B<Returns:> The configured C<ci.control_branch>, or the built-in default when
unset.
...
Note this is weaker than C<ci_configured>: enabled without a provider is a
half-configured repository, so check that instead before acting.

B<Parameters:> None.

B<Returns:> The configured C<ci.enabled> value.
...
This is the condition worth gating on. Enabled-without-provider is a
repository someone started configuring and did not finish, and treating it as
configured produces failures further along, where the cause is no longer
obvious.
...
B<Examples:>
    ...
    my $bool = $top->ci_configured();
    # returns false in a repo with ci.enabled but no provider
