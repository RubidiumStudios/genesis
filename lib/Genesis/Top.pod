...
C<provider> hash, which is optional even once the gate is on and carries the
automation's C<type>, C<target>, C<url>, C<team>, and C<insecure>, and
C<name>, which is the pipeline's own label in its provider and falls back to
the deployment type.

The enum for C<pipeline.provider.type> is built from
C<< Genesis::CI::Compiler::PipelineProvider->known_providers >>, so the schema
and every class lookup read one list and cannot spell a provider two ways.
The type defaults to C<manual>, and the C<provider> block itself defaults to
an empty hash so that the type's own default still applies when the operator
writes no provider block at all. A section that is enabled with no provider
block is therefore a manual pipeline rather than no pipeline.
