...
skipped, and so is one that will not load, so the merge is over what the
root actually holds. No C<Genesis::Env> is built and no kit is needed, which
is what lets the check run as the configuration loads.
...
own refusals with their keys named as the environment file spells them, and
the two shape refusals above.
...
B<Returns:> A list of one-line error strings.
