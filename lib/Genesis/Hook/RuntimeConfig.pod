...
B<(optional)> When true, C<perform> synthesizes the requested configs and
returns them (see L</collect_configs>) instead of printing, comparing, or
uploading them. Cannot be combined with C<print> or C<remove>. Defaults to
C<0>.
...
C<"Cannot specify the 'collect' option with the 'print' or 'remove' options.">
-- raised via C<bail()> if C<collect> is set together with C<print> or
C<remove>.
...
The dry-run flag. When true, the hook builds and compares configs but does
not upload or remove anything.

B<Parameters:> None.

B<Returns:> 1 when dry-run is active, 0 otherwise (default 0).
...
B<Examples:>
    ...
    # returns 1 when dry-run mode is active, 0 otherwise

=head2 interactive

    my $bool = $hook->interactive;

The interactive flag. When true, the hook prompts for confirmation before
each config upload.

B<Parameters:> None.

B<Returns:> 1 when interactive, 0 otherwise (default 0).

B<Examples:>

    my $bool = $hook->interactive;
    # returns 0 unless the caller asked for prompts

=head2 print

    my $bool = $hook->print;

The print flag. When true, generated runtime config content goes to the
terminal instead of being uploaded. Mutually exclusive with C<remove>;
C<init> bails if both are set.

B<Parameters:> None.

B<Returns:> 1 when printing, 0 otherwise (default 0).

B<Examples:>

    my $bool = $hook->print;
    # returns 1 when the caller asked to see the config rather than ship it

=head2 remove

    my $bool = $hook->remove;

The remove flag. When true, the hook removes the requested runtime configs
from the BOSH director rather than generating and uploading them. Mutually
exclusive with C<print>.

B<Parameters:> None.

B<Returns:> 1 when removing, 0 otherwise (default 0).

B<Examples:>

    my $bool = $hook->remove;
    # returns 1 on a removal run

=head2 collect

    my $bool = $hook->collect;

Returns the collect flag: true when C<perform> should synthesize the
requested configs and hand them back instead of acting on the director.
Defaults to C<0>.

B<Returns:> C<1> when collect mode is on, otherwise C<0>.

=head2 bosh

    my $bosh = $hook->bosh;

The BOSH client resolved during C<init>. When the environment is itself a
BOSH director this is the self-referencing target BOSH object; otherwise it
is the environment's associated director client.

B<Parameters:> None.

B<Returns:> A C<Service::BOSH::Director> object.
...
The C<Service::Credhub> object built from the BOSH director during C<init>.

B<Parameters:> None.

B<Returns:> A C<Service::Credhub> object, or C<undef> when the director has
no associated CredHub.

B<Examples:>

    my $credhub = $hook->credhub;
    # returns undef when the director exposes no CredHub

=head2 credhub_base

    my $base = $hook->credhub_base;

The CredHub base path used for secret entombment.

All three conditions must hold or this returns C<undef>: Genesis feature
compatibility of at least C<3.0.0-rc.1>, the C<genesis.entomb> manifest key
set to a true value, and a CredHub object that reports a base path. A
C<undef> here is the signal that entombment is off, not an error.

B<Parameters:> None.

B<Returns:> The base path string, or C<undef>.
...
B<Examples:>
    ...
    # returns '/bosh-genesis-kit/bosh-us-east-1/', or undef when entombment is off
...
When collection is requested (C<collect> is true), delegates entirely to
L</collect_configs> and returns its result before anything else happens.
...
=head2 collect_configs

    my $configs = $hook->collect_configs;

Synthesizes every requested runtime config by calling C<build> for it, and
records the results with C<done> without printing, comparing, uploading, or
storing secrets.  This is how C<genesis bosh-configs> obtains the runtime
configs a kit would upload, so it can report and diff them alongside the
cloud and cpi configs.

B<Returns:> An array reference, in request order, of hashes with these keys:

=over 4

=item C<build>

The build name that was requested (for example C<dns>).

=item C<name>

The BOSH config name the build uploads as, from C<config_name_for>.

=item C<description>

The registered description of the build.

=item C<content>

The synthesized YAML string, or C<undef> when the build failed or was
skipped (C<build> has already reported why).

=back

The same array reference is available afterwards from C<results>.

B<Examples:>

    $hook->validate_runtime_config_requests;
    my $configs = $hook->collect_configs;
    # Returns: [{build => 'dns', name => 'my-env.bosh.dns',
    #            description => 'Dns', content => "releases:\n..."}]
...
The confirmation prompt passes C<< $invert => 1 >> to
L<Genesis::UI/prompt_for_boolean>, so the truthy branch is the one that
B<skips>. That reads backwards at a glance, and is deliberate: the branch
body is the early return, so "yes" falls through to the upload and "no"
takes the skip. C<remove_configs> uses the same inversion. Only the default
differs -- upload defaults to yes, removal to no.
