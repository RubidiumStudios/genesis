...
name on C<Genesis::Hook::RuntimeConfig>.
...
"nothing to do" convention shared by the post-deploy step methods.
Failures in the underlying hook, upload, or vault write report by raising
...
hook complete is the caller's decision, not a side effect of one step.)
