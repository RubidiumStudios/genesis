...
B<Completion is the subclass's job.> C<Genesis::Hook::Check> never calls
C<< $self->done >> itself. Being an abstract base that delegates C<perform>
to its subclasses, it has no way to know when a check has finished or what
its result was. Each subclass must call C<< $self->done($result) >> before
returning from C<perform>; a subclass that forgets leaves the hook in an
incomplete state and C<completed> reports false.

This is the intended lifecycle for this class, not an oversight -- worth
stating because the absence of any C<done> call in this file reads like
one.
...
B<Returns:> In list context, a list beginning with the boolean result
(C<1> or C<0>), followed by a C<sprintf>-style format string for the
display message, followed by one C<sprintf> argument per C<%s>
placeholder in the format string.

The value count MUST match the format's placeholder count.  C<has_entry>
composes an C<info()> call from these return values, and an
under-supplied values list dies at C<Genesis::Log:1092> with
"Missing argument in sprintf".

Concretely, for C<$type eq 'params'> every branch returns
C<< ($ok, "params.#Y{%s}%s", $name, $qualifier_msg) >> -- four
elements, matching the two C<%s> slots.  For C<$type eq 'exodus'>
without env/deployment scoping the return is
C<< ($ok, "#Y{%s} exodus entry exists", $entry_name_or_msg) >> --
three elements, one C<%s>.  With env/deployment scoping the return
is C<< ($ok, "#Y{%s} exodus entry exists under #M{%s}", $entry_name_or_msg, $for) >>
-- four elements, two C<%s>.
