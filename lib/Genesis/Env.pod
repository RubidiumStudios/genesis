...
=head2 deployment_cache_preserve

    $env->deployment_cache_preserve;

Copies the cached state and store files (when present) from the deployment
cache into the repository's C<.genesis/manifests> directory, creating that
directory if needed.  Files missing from the cache are skipped, and a copy
failure emits a warning rather than dying.  Does nothing if the deployment
cache has not been set up.

This runs after a successful deployment, and before the cache is cleaned
up after a failed one: a failed C<bosh create-env> can still have created
or changed cloud resources, and the cached state file is the only record
of them.
