...
inherited by every environment beneath it. This key was C<required_files>
before D24 renamed it, and nothing but the name changed.
...
Calls C<bail("genesis.pipeline.track_additional_files entry #C{%s} escapes the deployment root.")> for any entry that is absolute, begins with C<~>, or contains a C<..> segment.  Config errors here surface loudly rather than silently resolving outside the repository.
