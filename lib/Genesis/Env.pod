...
=head2 _last_manifest_integrity_issue

    my $issue = $env->_last_manifest_integrity_issue($last_manifest);

Reports whether the manifest stored in F<.genesis/manifests/> still matches
the one Exodus recorded for the last deployment, so C<create-env> can warn
before it re-converges a VM using a state file that may be equally stale.

Compares the C<manifest_sha1> recorded in Exodus against the sha1
C<last_deployed_manifest> computed for the local manifest file.  Manifests
retrieved from a deployment archive are skipped: they travel with the
deployment, so there is no local copy that could have drifted.
...
The hashref returned by C<last_deployed_manifest>.

=back

B<Returns:> a description of the problem, or C<undef> when the manifest is
consistent, absent, or not subject to the check.

B<Examples:>

    my $issue = $env->_last_manifest_integrity_issue($last_manifest);
    # returns undef when the local manifest matches what was deployed
