...
The C<cpi> entry auto-appended by C<required_configs> is treated as
opportunistic: if the parent BOSH director cannot be reached (as when
kit-validator, dev iteration, or air-gapped smoke tests run without a
director in exodus or C<~/.bosh/config>) AND cpi is the only missing
config, the download is skipped silently.  Any non-cpi missing config
is a genuine kit requirement and the director-unreachable error is
