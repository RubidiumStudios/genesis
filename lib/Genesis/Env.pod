...
B<Bails> under the same conditions as L</cpi_az_map>.
...
B<Bails> when an AZ referenced by an instance group is absent from the
cloud-config.  BOSH cannot place a VM there and Genesis cannot determine which
CPI it would need, so dropping it silently would let a broken environment pass
preflight and fail at deploy time instead.
...
Entries are resolved relative to the Genesis top directory and returned
prefixed to be git-root-relative.
...
B<Errors:> Bails when called while the environment's own branch is checked
out -- files are copied B<into> it from the control branch's HEAD, so the
caller must be on control.
...
C<authenticate()> picks up the renewer through C<_on_auth_success>.
...
C<validate_create_env_state> have been converted from named package-level
subs to lexical anonymous subs inside C<use_create_env>, eliminating the
stale closure problem.  They are now internal helpers of that method and
are not callable from outside it.
