...
=head2 boolean_to_yaml
    ...
    my $yaml_str = Genesis::CI::Legacy::boolean_to_yaml($value);

Renders a Perl truth value as the YAML literal C<true> or C<false>.
Any true value yields C<true>, anything else -- including C<undef> and
the empty string -- yields C<false>.

Distinct from C<yaml_bool>, which normalises a possibly-absent setting
and returns a Perl value; this one produces the YAML text.
...
The value to test for truth. Not required to be defined.

=back

B<Returns:> The string C<true> or the string C<false>. Never C<undef>.
...
B<Examples:>
    ...
    Genesis::CI::Legacy::boolean_to_yaml(undef);
    # Returns: false
...
=item C<"Environment '$b' is already being triggered by environment '$triggers->{$b}'.\nIt is illegal to trigger an environment more than once.\n"> -- an environment appears as a trigger target from a different source. Identical duplicate declarations (same source and target) are silently accepted.
...
=item C<pipeline.require-passed-caches> default will change from C<true> to C<false>

The C<require-passed-caches> pipeline key currently defaults to
C<JSON::PP::true> during validation. A planned change will invert this
default to C<JSON::PP::false>. Pipelines that rely on the current default
should explicitly set C<require-passed-caches: true> to preserve behavior
after the change.
