...
named artifact from the deployment and returns there.  When
C<--fetch-artifacts-to> is set, downloads deployment artifacts to the
specified directory and returns there.  When
...
B<Returns:> C<0>, or C<1> where C<--fetch-artifacts-to> could not fetch one
of the artifact types it found.  C<info> declares C<DEPLOYED_STATE>, so it
runs inside the branch session the gate opened and hands its status back
rather than exiting with it, and C<run_command> exits on what it returns once
that session is closed.  A call with the wrong number of arguments exits 2
through C<command_usage>, and every other way the function can fail bails
instead of returning, so C<0> is a run that did what it was asked.
...
B<Examples:>
  ...
  # prints nothing and bails: The 'deployments' command is not yet
  # fully implemented.
...
=head2 _format_feature_compatibility_line($deployment)

Formats the feature opt-in line shown for a deployment, or nothing when the
deployment records no opt-in.

The source is reported only when the record carries one. Older deployment
records predate the source being tracked, so the level appears bare rather
...
A C<Genesis::Env::Deployment> object.

=back

B<Returns:> The formatted line including its trailing newline, or the empty
string when the deployment has no C<feature_compatibility> recorded.
Returning C<''> rather than C<undef> lets the caller concatenate it
unconditionally.

B<Examples:>

  my $line = _format_feature_compatibility_line($deployment);
  # returns "  feature opt-in v3.1.0 (from environment)\n"

  # A deployment recording a level but no source:
  my $line = _format_feature_compatibility_line($old_deployment);
  # returns "  feature opt-in v3.0.0\n"

  # A deployment with no opt-in at all:
  my $line = _format_feature_compatibility_line($plain_deployment);
  # returns '' -- nothing is printed for it
