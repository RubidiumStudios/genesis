...
named in passing from matching, so the commit's own sha is taken off the
front of each line before the line is read.
...
Arrayref of log subject lines to scan, each in git's C<%H %s> shape, which is
what C<log_subjects> returns by default. C<undef> is treated as empty.
