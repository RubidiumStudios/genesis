...
The commit message carries a C<[pipeline] control@E<lt>shaE<gt>> marker,
which is what C<_format_pipeline_reason> later reads to explain a deploy.
The two are a pair: changing the format here breaks reason derivation there.
