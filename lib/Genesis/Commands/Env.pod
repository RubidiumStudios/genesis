...
the branch the operator is standing on.  It creates no deployment branch
and reconciles none, because C<genesis pipeline-apply> is the only command
that creates one, and a C<genesis propagate> run is what delivers the
commit onto it.
...
=item C<"Creating environments requires being on the #C{%s} branch, but you are currently on #C{%s}.">

Raised when the repository uses a control branch and the working tree is on
some other branch. The environment file belongs on control, where the
topology is read from, so committing it while standing on another
environment's branch would put it somewhere nothing reads it. The first
C<%s> is the control branch name, the second the branch actually checked
out.
...
The first three serve C<deploy>'s reason line; the last three serve the
pipeline pull, where an environment picks up propagation files from its
predecessor.
