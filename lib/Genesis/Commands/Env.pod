...
a stale one; it reports what it found and returns false.  A live lock stops
both a dry run and a real deploy, since a deploy in progress is changing the
claims being read.  A stale lock is cleared before a real deploy acquires
...
Raised (via C<bail()>) when another process holds a live lock, for a dry
run and a real deploy alike.
