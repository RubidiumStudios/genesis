...
some other branch. Environment branches are created I<from> control, so
creating one while standing on another environment's branch would base it on
that environment rather than on control. The first C<%s> is the control
branch name, the second the branch actually checked out.
...
Raised when the environment has no branch in the repository. Both C<%s> are
the branch name; the message names the command that creates it.
