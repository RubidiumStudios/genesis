...
Raised when a pipeline deploy is attempted with a dirty working tree. The
deploy records the commit it ran from, so uncommitted changes would be
deployed but not recorded, leaving the audit trail describing something
other than what shipped.

=item C<"Environment branch #C{%s} does not exist.\nCreate it with #C{genesis pipeline-apply} on the control branch.">

Raised when the environment has no branch in the repository. The C<%s> is
the branch name, and the message names the one command that creates a
deployment branch.
...
reason line, and C<_assert_prior_env_deployed>, C<_get_source_sha_for_pull>,
and C<_apply_pull_propagation> serve the pipeline pull, where an environment
picks up propagation files from its predecessor. The rest each serve one
step of a deploy.
...
=head2 _get_source_sha_for_pull($env, $prior_name, $git)

Determines which control-branch commit an environment should pull its
propagation files from.

With no predecessor the environment is the pipeline's entry point, and pulls
from control HEAD. Otherwise it pulls from the control commit recorded by the
predecessor's last successful deployment -- so it receives exactly what the
predecessor was deployed with, not whatever has landed on control since.

That distinction is the whole point of the pipeline: an environment should
inherit a known-good state, not the newest one.

Deployments are examined newest-first, and C<post-failed> counts as
successful for the same reason as in C<_assert_prior_env_deployed>. When no
deployment records a control commit, the function warns and falls back to
control HEAD -- which loses the guarantee above, hence the warning.
...
The C<Genesis::Env> pulling.

=item C<$prior_name>

The predecessor's name, or a false value for the entry point.
...
B<Returns:> The sha to pull from.

B<Examples:>

  my $sha = _get_source_sha_for_pull($env, undef, $git);
  # returns control HEAD -- this env is the pipeline entry point

  my $sha = _get_source_sha_for_pull($env, 'staging', $git);
  # returns the control commit staging was last deployed with

  my $sha = _get_source_sha_for_pull($env, 'staging', $git);
  # warns and returns control HEAD when staging recorded no
  # git.control_commit

=head2 _apply_pull_propagation($env, $env_branch, $source_sha, $git)

Copies the environment's propagation files from C<$source_sha> onto its
branch and commits the result.

Deletions are carried across as well as changes: a file removed on control
must be removed here too, or the environment keeps configuration the pipeline
has retired.

Three outcomes, each reported rather than silent -- an operator running this
should be able to tell "nothing to do" from "did nothing":

=over 4

=item *

The environment declares no propagation files, so there is nothing to pull.

=item *

It declares them but is already current, so there is nothing to change.

=item *

Files differ, and the count pulled is reported along with the source commit.

=back

The commit message carries the marker L<Genesis::CI::Marker/build> renders,
with the word C<(pulled)> after it, and C<_format_pipeline_reason> is what
reads that marker back to explain a deploy.  The format lives in that module
rather than here, so the writer and the reader cannot drift apart.
...
The environment's branch name.

=item C<$source_sha>

The control commit to pull from.
...
B<Returns:> Nothing. Reports progress to the terminal and commits to the
repository.

B<Examples:>

  _apply_pull_propagation($env, 'staging', $sha, $git);
  # prints "staging: pulling 3 files from control@a1b2c3d" and commits

  _apply_pull_propagation($env, 'staging', $sha, $git);
  # prints "staging: already current" and commits nothing
