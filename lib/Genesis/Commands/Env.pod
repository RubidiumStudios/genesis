...
Only lines carrying a C<[pipeline] control@E<lt>shaE<gt>> marker count. Those
markers record where a propagation came from, so the useful reason is the
subject of the I<control> commit, not of the propagation commit that carried
it across.
...
Arrayref of log subject lines to scan. C<undef> is treated as empty.
