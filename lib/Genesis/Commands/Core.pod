...
Reads the deployment repository's configuration from
...
B<Returns:> C<0> when the key was found (or when dumping), C<1> when the
key is not set.
...
=head2 C<config> is read-only

Writing is not implemented.  L<Genesis::Config/set> and
L<Genesis::Config/save> exist, but C<set> does not yet validate against
the schema, so a write path would happily persist a value that
L<Genesis::Top> refuses to load afterwards.
