...
bailing with migration instructions.
...
block, and the C<genesis config> invocation that sets the two keys the gate
checks.  It says plainly that the rest -- git URI, branch, pipeline name and
vault URL -- must be written into F<.genesis/config> by hand, the schema not
yet declaring them.
