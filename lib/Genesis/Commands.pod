...
lock. An exit hook returns the operator to the branch they stood on, calling
C<finish> after a clean run and C<abort> after a failed one.
...
runs the two gates that stand in front of every command, then calls the
registered function with C<@COMMAND_ARGS> and exits with status 0 on
success.

The gates run in order.  C<_gate_pipeline_on_legacy_ci_yml> refuses a
pipeline command in a repository that still carries a legacy F<ci.yml>, and
C<_gate_branch_class> enforces the branch class the command declared at its
registration.  Either one can bail, and neither returns a value the dispatch
reads.
...
C<run_command> just after the legacy F<ci.yml> gate and before the command's
own function.
...
Returns early -- allowing the command through -- in five cases: the command
declares no class; it declares C<branch_target =E<gt> 'control'>, which
C<propagate> does because it switches to control inside its own session; it
has no repo or env scope, so there is no C<Genesis::Top> to read; the
C<Top> or the git handle could not be loaded; or the repository has no
pipeline enabled, where every command behaves as it always has.
...
B<Returns:> nothing.  Bails when the gate refuses.
...
B<Examples:>
  ...
  _gate_branch_class();
  # no return value; bails when a pre-deploy command stands on a
  # deployment, pull request, or artifacts branch, on a feature branch
  # that does not descend from control's tip, or on one named for an
  # environment

=head2 _gate_deployed_state

  my $session = _gate_deployed_state($top, $git);

Opens a branch session on C<$git> and switches it to the deployment branch
of the environment the command was given, which is C<< <env>/<type> >>.
...
argument at all switches nothing and returns.

An exit hook registered through C<at_exit> closes the session once the
command is done.  A clean run calls C<finish>, which restores the branch
C<begin> recorded and releases the switch lock, and a run that died calls
C<abort>, which discards what the command left behind first.
...
session is told never to reset and for the deployment branch to switch to.
...
B<Returns:> the open C<Service::Git::Session>, or nothing when the command
was given no environment to switch to.

B<Examples:>

  _gate_deployed_state($top, $git);
  # `genesis qa info` is switched to qa/bosh and returned afterwards

  _gate_deployed_state($top, $git);
  # `genesis envs/qa.yml info` strips the path and switches to qa/bosh
