...
one command that declares it, with the value C<'control'>.  Defaults to
C<undef>.
...
calls the registered function with C<@COMMAND_ARGS> and exits with
status 0 on success.
...
declares no class; it has no repo or env scope, so there is no
C<Genesis::Top> to read; the C<Top> could not be loaded; the repository has
no pipeline enabled, where every command behaves as it always has; or the
command declares C<branch_target =E<gt> 'control'>, which C<propagate> does
because it switches to control inside its own session.
