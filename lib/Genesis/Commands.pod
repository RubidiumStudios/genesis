...
The refresh is passed along as C<< refresh => 0 >> when the running command
carries C<--no-refresh>, which C<pipeline-status> alone declares.  That
command promises to make no network call, so the flag is read here, where the
options are, rather than in the assertion, which has no business knowing
which command declared it.
