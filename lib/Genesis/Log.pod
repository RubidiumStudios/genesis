...
For each C<{name}> in the template, if C<name> is a key in C<%concrete>
the value is substituted directly; otherwise the variable is replaced
with a shape-matching wildcard. This lets callers control the grouping
