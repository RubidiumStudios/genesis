...
Everything below documents the B<shell library> this module emits, not its
Perl interface -- that is C<write> and C<log_from_script>, above, and nothing
else. These are bash functions living in the C<__DATA__> section, written out
and sourced by kit hook scripts; they are not callable from Perl and have no
Perl subs behind them.

They are available in every Genesis kit hook script after Genesis writes and
sources the helper library, exported to the shell environment via
C<export -f>.
...
=head1 INTERNAL BASH HELPERS

Also bash, also from the emitted library. These are used internally by the
helper library itself; kit hook scripts should not call them directly.
