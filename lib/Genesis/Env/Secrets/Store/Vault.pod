...
A list of key name strings, or an empty list when the cache is loaded and no
arguments are given.
...
The bulk data cache is cleared unconditionally before returning, regardless
of whether the write succeeded or encountered an error. The secret's stored
