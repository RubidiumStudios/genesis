...
When C<keys()> reads from the cache it returns bare key names, via Perl's
built-in C<keys>. When the cache is not loaded, or the path falls outside the
store's base, the call is delegated to C<< $store->service->keys() >> and the
results come back in the service's native C<path:key> form.

A caller therefore gets different shapes depending on whether the cache
happens to be populated, and a single call can return B<both> shapes at once
when some paths are cached and others are not. Do not assume the format;
normalise what comes back, or ensure the store is loaded first.
