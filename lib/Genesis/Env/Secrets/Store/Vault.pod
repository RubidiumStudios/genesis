...
=item * If the cache B<is> loaded, returns B<all cached paths> from the store
data.
...
A list of absolute Vault path strings. When called with no arguments, returns
all paths (either from the cache or from the live service).
...
=item * If the cache B<is> loaded, returns B<all cached keys> from the store
data. Because C<keys()> internally calls C<paths()> with no arguments, which
returns all cached paths, all keys from those paths are collected and returned
as bare key names.
...
(cache not loaded). When called with no arguments and the cache is loaded,
returns bare key names for all cached paths. See L</KNOWN ISSUES> for the
format inconsistency between cached and delegated returns.
...
B<Examples:>
    ...
    $store->store_data;
    my @all = $store->keys;
    # Returns: all cached key names (bare names, not path:key format)
