...
Genesis::Env::Secrets::Store - Abstract base class for secrets store backends

=head1 DESCRIPTION

Provides a factory method and abstract interface for secrets store backends.
Subclasses implement concrete storage operations against specific services
(e.g., Vault, Credhub).

=head1 CLASS METHODS

=head2 provide($env, $service, %options)

Factory method that returns the appropriate secrets store subclass instance
based on the service type.

  my $store = Genesis::Env::Secrets::Store->provide($env, $env->vault);

Supported service types:

=over

=item * C<Service::Vault::Remote>, C<Service::Vault::Local> - returns a C<Genesis::Env::Secrets::Store::Vault>

=item * C<Service::Credhub> - returns a C<Genesis::Env::Secrets::Store::Credhub>

=back

Options are passed through to the subclass constructor (e.g., C<mount_override>,
C<slug_override>, C<root_ca_override>).

=head1 ABSTRACT METHODS

Subclasses must implement the following:

C<new>, C<default_mount>, C<mount>, C<default_slug>, C<slug>, C<label>,
C<list>, C<get>, C<set>, C<authenticate>, C<is_authenticated>, C<is_available>,
C<generate>, C<validate>, C<regenerate>, C<remove>, C<remove_all>
