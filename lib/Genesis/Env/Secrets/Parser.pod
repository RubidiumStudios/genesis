...
The environment this parser was constructed against.

B<Parameters:> None.

B<Returns:> The C<Genesis::Env> passed to C<new>, or C<undef> when none was
given -- which is legitimate for subclasses that take kit metadata and
feature lists directly as C<parse> options instead.

B<Examples:>

  my $env = $parser->env;
  # returns the Genesis::Env object

  my $env = $parser->env;
  # returns undef for a parser constructed without one
