...
The environment the plan was created for.

B<Parameters:> None.

B<Returns:> The C<Genesis::Env>, or C<undef> when none was passed to
C<new()>.
...
The Vault-backed secret store the plan reads and writes through.

B<Parameters:> None.

B<Returns:> The store object passed to C<new()>.
...
The CredHub handle the plan uses for entombed secrets.

B<Parameters:> None.

B<Returns:> The CredHub object passed to C<new()>.

B<Examples:>

    my $credhub = $plan->credhub;
    # returns the Service::Credhub object

=head2 parent

    my $parent = $plan->parent;

The plan this one was filtered from.

C<filter()> produces a new plan holding a subset of the secrets, with the
original recorded here -- so a filtered plan can still reach the full set it
came from.

B<Parameters:> None.

B<Returns:> The parent plan, or C<undef> for a root plan.

B<Examples:>

    my $parent = $plan->parent;
    # returns undef on a plan built directly by new()

=head2 secrets

    my @secrets = $plan->secrets;

Every secret in the plan, B<in dependency order>: certificate authorities
before the certificates they sign, then all non-X.509 secrets by path.

The order is the point. Generating a signed certificate before its CA exists
fails, so anything walking this list to create secrets depends on the
ordering being correct rather than incidental.

B<Parameters:> None.

B<Returns:> The list of C<Genesis::Secret::*> objects.

B<Examples:>

    my @all = $plan->secrets;
    printf "Plan contains %d secrets\n", scalar @all;
    # returns the secrets, CAs first

=head2 paths

    my @paths = $plan->paths;

The vault paths of every secret in the plan, sorted.

Note this is sorted by path, B<not> in the dependency order C<secrets> uses.

B<Parameters:> None.

B<Returns:> The sorted list of path strings.

B<Examples:>

    my @paths = $plan->paths;
    # returns ('env/name/secret/ca', 'env/name/secret/server', ...)

=head2 secret_at

    my $secret = $plan->secret_at($path);

The secret at a given vault path.
...
The vault path to look up.

=back

B<Returns:> The C<Genesis::Secret::*> object, or C<undef> when the plan holds
...
The secrets that failed to parse or order -- those that came back as
C<Genesis::Secret::Invalid>.

Invalid secrets stay B<in> the plan rather than being discarded, so that
every problem can be reported at once instead of one per run. This is the
accessor that finds them.

B<Parameters:> None.

B<Returns:> The list of invalid secrets. Empty when the plan is sound.
...
B<Examples:>
    ...
    # returns the Genesis::Secret::Invalid objects
...
Gets or sets the verbosity flag. Modifies the object in place when called
with an argument.
...
B<(optional)> Any true or false value. Omit to read.

=back

B<Returns:> 1 when verbose is enabled, 0 otherwise -- normalised, so the
value read back is not necessarily the one written.
