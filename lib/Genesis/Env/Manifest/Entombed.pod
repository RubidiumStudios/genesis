...
True here: entombment leaves C<((credhub-var))> references that the director
resolves at deploy time, so nothing secret is present but nothing is missing
either.
...
=head2 redacted
  ...
  my $manifest = $manifest->redacted();

Returns the manifest unchanged.

Redaction exists to strip vault secrets before a manifest is shown or
stored. An entombed manifest never held any -- they were replaced by credhub
references during entombment -- so there is nothing to remove and the object
returns itself rather than building a second copy.
...
B<Examples:>
  ...
  # returns the same object -- already free of vault secrets
