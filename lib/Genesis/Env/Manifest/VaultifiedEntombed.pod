...
Whether this manifest variant can be handed to BOSH as-is.

True here: the vaultified secrets have been entombed into Credhub, leaving
C<((credhub-var))> references the director resolves at deploy time.
...
The files merged to produce this manifest, in merge order: the initiation
file, the kit's files, the cloud config, the environment files, and the
conclusion file.

Cloud config is required here rather than optional -- a deployable manifest
needs the IaaS shape resolved, so a missing cloud config is an error rather
than a section left unfilled.

B<Parameters:> None.

B<Returns:> The list of file paths, in merge order. Note this returns a
list, where the sibling C<Entombed> returns an arrayref.

B<Examples:>

  my @files = $manifest->source_files();
  # returns the initiation file, kit files, cloud config,
  # environment files and conclusion file, in that order
