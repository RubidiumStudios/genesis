...
Cloud config is B<optional> here, unlike in the deployable variants. This
manifest is not evaluated, so an absent cloud config leaves operators
unresolved rather than producing a broken result -- which is exactly what
this variant is for.

B<Parameters:> None.

B<Returns:> An arrayref of file paths, in merge order.

B<Examples:>

  my $files = $manifest->source_files();
  # returns [ initiation, kit files..., cloud config, env files..., conclusion ]

  # In a repo with no cloud config:
  my $files = $manifest->source_files();
  # returns the same list without it -- optional, not an error
