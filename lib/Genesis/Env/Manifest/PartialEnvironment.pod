...
The environment files alone -- no initiation file, no kit files, no cloud
config, no conclusion.

That is the whole point of this variant: it shows what the operator wrote,
merged across the environment's inheritance chain, without anything the kit
contributes. Adding kit files would make it impossible to tell which
settings came from where.

B<Parameters:> None.

B<Returns:> An arrayref containing the environment files, in merge order.

B<Examples:>

  my $files = $manifest->source_files();
  # returns [ 'envs/base.yml', 'envs/staging.yml' ] -- environment files only
