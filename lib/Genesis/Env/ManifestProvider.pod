...
B<Errors:> Spruce's own merge failures come back in C<$errors> rather than
raising -- callers are expected to read them and decide. Bails only when
the spruce invocation itself cannot be completed, naming the manifest type
and environment; that is a broken toolchain rather than a bad manifest, and
there is nothing for the caller to recover from.
...
=head2 cached_or_build

  my $manifest = $provider->cached_or_build(@types);

Returns the first of the named manifest types that has already been built,
building the first named type only if none of them has.

The point is to avoid paying for a merge that a cheaper variant can answer.
A caller wanting whichever of several manifests is already to hand names them
in preference order and gets one without forcing work.

The special type C<deployment> resolves to the environment's actual
deployment manifest type. Because that can resolve to a type also named
explicitly -- a non-vaultified environment deploys its C<unredacted>
manifest -- the list is de-duplicated after resolution, so the same type is
not considered twice.

A type counts as built when it has either data or a file; a manifest object
that exists but holds neither is not a cache hit.
...
Manifest type names in preference order. At least one is required.

=back

B<Returns:> The first already-built manifest among C<@types>, or a freshly
built manifest of the first type when none is.
...
C<"cached_or_build: at least one manifest type required"> -- raised via
C<bug()> when called with no types.

=back

B<Examples:>

  my $m = $provider->cached_or_build(qw/entombed unredacted/);
  # returns the entombed manifest if already built, else unredacted if
  # that one is, else builds entombed

  my $m = $provider->cached_or_build('deployment');
  # returns the environment's deployment manifest type, resolved
