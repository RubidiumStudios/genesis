...
B<Examples:>
  ...
  # timestamps of the 5 most recent successful deployments
...
B<Examples:>
  ...
  # most recent successful deployment of any action
  my $d = $mgr->latest();
  print $d->action if $d; # prints 'deploy' or 'terminate'

  # most recent attempt regardless of result
...
B<Examples:>
  ...
  # synthesize from the current exodus store
  my $d = $mgr->synthesize_from_exodus();
  if ($d) {
    print $d->action; # prints 'deploy'
  }

  # synthesize from previously fetched exodus data
...
On a repository that has a pipeline, the record also carries a C<git> block,
which is built by C<_git_context()> and holds the branch, the commit the
deploy stood on, and the control commit that branch's newest marker names.
The branch and the commit are stored before the marker is read, so a marker
read that fails costs the control commit alone and leaves the rest of the
block standing. The whole read is wrapped in an C<eval> as well, and nothing
at all is recorded when the repository cannot be opened, because the git
context supplements the record rather than making it.
...
The git block of a deployment audit record, which is the branch, the commit
the deploy stood on, and the control commit that branch's newest marker
names.

The two hashes are a pair and are recorded together.  C<commit> is the
deployed commit, which is what actually ran, including the case where an
operator pushed a hand commit to the branch and deployed it.
C<control_commit> is the certified commit, which is the control commit whose
mirror was deployed, and it is read with L<Genesis::CI::Marker/newest>, so a
hand commit above the marker is skipped and a squash merge's body is still
read.

C<control_commit> is absent on a branch carrying no marker at all.  Such a
branch has been delivered nothing, and recording the control branch's tip in
its place would turn a label into a claim.  It is absent too when the marker
walk itself fails, because that read is guarded on its own so that the
branch and the commit still come back.
...
A C<Service::Git> handle on the repository the deploy is standing in.

=back

B<Returns:> A hashref of C<branch>, C<commit>, and, where a marker stands,
C<control_commit>.

B<Examples:>

  Genesis::Env::DeploymentManager::_git_context($git);
  # returns {branch => 'qa/bosh', commit => '...', control_commit => '...'}
...
B<Examples:>
  ...
  # integer range returns deferred index list
...
B<Examples:>
  ...
  # expand a partial year to an end-of-year boundary
