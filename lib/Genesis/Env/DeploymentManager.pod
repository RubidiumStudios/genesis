...
C<['state']> or C<['state', 'manifest']>. Naming artifacts implies
C<include_failed>, because a record archived after a failed attempt can hold
newer state than the last success: C<bosh create-env> rewrites the state file
as it works, so a deploy that died partway still recorded what it changed.
