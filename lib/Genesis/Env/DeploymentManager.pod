...
The whole read is wrapped in an C<eval> and nothing is recorded when it
fails, because the git context supplements the record rather than making it.
...
its place would turn a label into a claim.
