...
=head2 latest_with_artifacts

  my $deployment = $mgr->latest_with_artifacts(%options);

Returns the most recent deployment audit that archived every artifact named,
scanning from newest to oldest. Unlike C<latest()>, this deliberately
considers failed records.

C<bosh create-env> rewrites the state file as it works, so an attempt that
died partway still recorded what it had already changed on the IaaS, such as a
deleted VM, an uploaded stemcell, or a fresh persistent disk. That state is
newer than the last successful deploy's and it is the only accurate account of
what is out there, so the next deploy and the next terminate both build on it.
...
Arrayref of artifact types or filenames that the record must carry. Defaults
to C<['state']>.
...
Thrown via C<bug()> for any option other than C<action> or C<artifacts>.
...
B<Examples:>
  ...
  my $d = $mgr->latest_with_artifacts(action => 'deploy');

  # terminate needs the manifest that goes with that state
  my $d = $mgr->latest_with_artifacts(
