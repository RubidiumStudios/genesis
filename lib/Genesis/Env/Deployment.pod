...
Returns true if the given result is considered successful (success or post-failed).

=head2 is_a_failed_result($result)

Returns true if the given result is considered failed.
...
Returns true if the deployment result was successful.
...
Builds a tarball of deployment artifacts.

=item _get_artifact_tarball()

Gets the artifact tarball for this deployment.

=item _artifact_map()

Returns the artifact map for this deployment.
...
Determines if a string is base64-encoded gzipped data.
