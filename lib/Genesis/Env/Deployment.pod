...
=head2 new([\%opts,] $env, %data)

Creates a new Genesis::Env::Deployment object based on the provided data.

Validation strictness depends on whether the record is being B<written> or
B<read back>.  A record Genesis is creating now must be complete -- a missing
field there is a bug in Genesis and is fatal.  A record loaded from vault is
untrusted input: it may have been written years ago by a Genesis predating the
current schema, or hand-edited.  Those are normalized and defaulted instead,
because the deployment they describe is already over and failing to parse it
must not obstruct the deployment happening now.

B<Parameters:>

=over

=item \%opts (optional, leading)

=over

=item from_storage

When true, treat C<%data> as a record being read back: recognize the legacy
flat format, fill unrecoverable required fields with C<'unknown'>, coerce an
unrecognized C<action>/C<result>, and record what was substituted for
L</is_synthesized()>.  Defaults to false.

Records being B<created> keep the strict checks -- a field missing there is a
bug in Genesis and must stay loud. Records being B<read> are untrusted input:
they may have been written years ago by a Genesis predating the current
schema, or hand-edited since. Refusing them would make old audit history
unreadable rather than merely incomplete.

When required fields are absent, all of them are collected before reporting,
so a caller that omitted C<action> gets the complete missing-field list
rather than a stream of uninitialized-value warnings from the checks that
follow.
...
=item reason (required when writing)

The reason for the deployment.  Exempt under C<from_storage>: the intermediate
C<state>-keyed legacy format never carried one, so requiring it on read made
every such record unloadable.

Note this is only the presence of the field.  Whether a I<meaningful> reason
is demanded is a separate policy --
C<Genesis::Env::deployment_change_reason_required_size_policy>, sourced from
OCFP config (C<policies.deployment_change_reason_required_size>) then repo
config, disabled by default -- enforced in the deploy and terminate paths
...
A hashref mapping artifact type names to local file paths (write side
on the way in).

=item *

A base64-encoded gzip string containing a tarball of artifact
contents (back-compat read shape for deployments stored before
chunking, or pre-chunk in-memory representation).

=item *

An array reference of base64-encoded gzip string B<chunks> (read
shape for chunked deployments). The constructor concatenates the
chunks in order to reconstitute the single base64-gzipped scalar
before further processing. This shape is produced by
L<Genesis/unflatten> when the vault stores the artifact across
C<artifacts[0]>, C<artifacts[1]>, ... keys.
...
=head2 is_synthesized()

Returns true when values were supplied by Genesis to make a stored record
readable, rather than being recorded at deployment time.

Only ever true for records constructed with C<from_storage>.  The deployment
described is real; some of what the object reports about it is inferred.

B<Returns:> 1 if any field was synthesized, 0 otherwise.

B<Examples:>

  warning("some details are inferred") if $deployment->is_synthesized;

B<Limitations:> A flag, not a provenance record -- it tells you the object
contains synthesized values but not what the original record actually said.
Use L</synthesized_fields()> to see which fields were affected, and the trace
log for the detail.

=head2 synthesized_fields()

Returns the names of the fields that were normalized or filled in for a stored
record: top-level names such as C<action> or C<genesis_version>, and the
container names C<kit>, C<manifest>, or C<user> when those were rebuilt from
the legacy flat form.

B<Returns:> A list of field names, empty when nothing was synthesized.

B<Examples:>

  print join(', ', $deployment->synthesized_fields);
  # prints kit, manifest, user
...
which is a relative path. That does not match the full exodus path
C<commit()> writes to, so this may report false for a deployment that has in
fact been committed -- see C<M#1> under L</KNOWN ISSUES>.
...
artifacts are present, they are packaged as a base64-encoded gzip tarball
and stored across one or more vault keys via vault's C<key@file> notation.

B<Chunked artifact storage:> the encoded tarball is split into chunks
sized to fit within Vault's per-JSON-string limit, then written as
C<artifacts[0]>, C<artifacts[1]>, ..., C<artifacts[N-1]> within a
single C<safe set> call. Chunk size is derived from
L<Service::Vault/max_json_string_value_length> minus 2 KiB of headroom
for key prefix, JSON envelope, and base64 padding. The relevant
upstream cap is Vault 1.21+'s C<max_json_string_value_length> listener
option (default 1 MiB), which caps each individual JSON string value
in the request body but not the body as a whole. Older deployments
written under the single-key C<artifacts> shape remain readable
unchanged.
...
mutation is a side effect of this method, and it is permanent -- see C<M#3>
under L</KNOWN ISSUES>.
...
Thrown via C<bail()> when the intermediate artifacts file cannot be reopened
for chunking, after C<_build_artifacts_file> has written it. C<%s> is the
temporary file path and C<$!>. Reaching this means the file was created but
became unreadable between writing and reading -- a full or unwritable
workdir, or the file being removed underneath the process.
...
=head2 has_artifact($artifact)

Returns true when this deployment carries the named artifact, looked up by
type name or by filename.  Unlike C<artifact()>, this asks a question rather
than fetching anything, so it is safe to call on a record that stored no
artifacts at all.
...
The artifact type name (e.g., C<'state'>) or filename
(e.g., C<'myenv-state.json'>) to look for.

=back

B<Returns:> 1 when the artifact is present, 0 when it is not.
...
Thrown via C<bug()> when C<$artifact> is missing or empty.

=back

B<Examples:>

    # Pick the newest deploy that archived a create-env state file
    my ($recovered) = grep {$_->has_artifact('state')} $env->deployments->all;
...
=head2 _normalize_legacy_fields

  my @synthesized = _normalize_legacy_fields(\%data);

Rewrites the original flat, snake_cased audit format into the nested one
C<new()> expects. Plain function, not a method. Modifies C<\%data> in place.

Three groups are converted, each only when the nested key is absent and at
least one legacy key is present, so a record already in the new shape passes
through untouched:
...
C<kit_id>, C<kit_name>, C<kit_version>, C<kit_is_dev>, C<kit_features> into
C<kit>. C<features> is accepted as a fallback source for C<kit_features>.

=item *

C<manifest_type>, C<manifest_sha2>, C<manifest_sha1> into C<manifest>.

=item *

C<deployer> into C<< user.shell >>.

=back

Note the keying: this runs on B<underscored> names, so the C<unflatten> pass
in C<new()> -- which only fires on dot notation -- never touches these.

The same translation exists in
L<Genesis::Env::DeploymentManager/synthesize_from_exodus> for the top-level
exodus blob. Having it here too means both read paths agree on the legacy
shape, rather than one understanding it and the other treating it as fatal.

B<The SHA-1 case is why this returns anything.> Legacy records stored a
SHA-1 digest under the C<sha2> name. Rather than silently presenting it as a
SHA-2, the digest is recorded under C<sha1> as well and flagged with
C<using_sha1>, so a caller does not compare hashes across digest types and
conclude a manifest changed when it did not.
...
The record being normalised. Modified in place: legacy keys are deleted and
their nested equivalents added.

=back

B<Returns:> The list of top-level field names that were synthesized -- some
of C<kit>, C<manifest>, C<user>. Empty when the record needed no conversion.
C<new()> uses this to distinguish a field it invented from one the record
actually carried.

B<Examples:>

  my %data = (kit_id => 'bosh/1.2.3', kit_name => 'bosh',
              kit_version => '1.2.3', deployer => 'dbell');
  my @synth = _normalize_legacy_fields(\%data);
  # returns ('kit', 'user'); %data now has nested kit and user hashes

  my %data = (kit => {id => 'bosh/1.2.3'});
  my @synth = _normalize_legacy_fields(\%data);
  # returns () -- already in the nested shape

=head2 _apply_unknown_defaults

  _apply_unknown_defaults(\%data, \@missing);

Fills in placeholder values for required fields a stored record does not
have. Plain function, not a method. Modifies C<\%data> in place.

Only reached for records read back from storage. A record being written keeps
the strict check and fails loudly, because a field missing there is a bug in
Genesis rather than an artefact of age. Stored records may have been written
years ago by a Genesis predating the current schema, or hand-edited; that is
untrusted input, and refusing to read it would make old audit history
inaccessible.

Defaults are per-field: C<action> becomes C<'deploy'>, C<result> the
"assumed" action result, C<user> / C<kit> / C<manifest> minimal hashes with
their own required sub-fields set to C<'unknown'>, and anything else the
string C<'unknown'>.
...
The record being defaulted. Modified in place.

=item C<\@missing>

The field names to fill. Fields already present are skipped, so passing a
name that turned out to exist is harmless.
...
B<Examples:>
  ...
  my %data = (genesis_version => '3.1.0');
  _apply_unknown_defaults(\%data, ['action', 'user']);
  # modifies %data in place: action => 'deploy',
  #                          user   => {shell => 'unknown'}

  my %data = (action => 'terminate');
  _apply_unknown_defaults(\%data, ['action']);
  # no change -- the field already exists
...
B<Errors:> Bails when the deployment record carries no compressed
artifacts, and again when the artifacts are present but cannot be
decompressed. Both are unrecoverable here: there is no partial tarball to
hand back, and returning C<undef> would push the same failure onto every
caller.
...
B<Errors:> Bails when the timestamp cannot be parsed, quoting the value it
was given. Exodus timestamps are written by Genesis itself, so an
unparseable one means the record is corrupt rather than merely unexpected.
...
Defects that are still present, recorded so callers do not build on the
current, incorrect behaviour. Resolved entries are removed rather than marked,
so everything listed here is live.

The C<C#>, C<M#>, C<m#> and C<I#> labels are anchors within this document --
severity and number, nothing more. They are not references to any tracker.

=head3 Critical

=over

=item C#3: List-context commit() bypasses error check (in commit())
...
=item M#1: committed() uses relative vault path (in committed())
...
=item M#2: user_colorized_legend breaks when invoked as a method (in user_colorized_legend())
...
=item M#3: commit() permanently mutates $self->{data} (in commit())
...
=item M#4: Vault set arguments unescaped (in commit())
...
=item M#5: secrets/secrets.json dual-key handling fragile (in _get_artifact_hash())
...
=item m#1: Typo "TDB" should be "TBD" (in new())

Comment reads C<# TDB: Should we automate...> instead of C<# TBD: ...>.

=item m#2: Sub-field validation skips when parent has wrong type (in new())
...
=item m#3: _get_ts_string called before timestamp conversion (in new())

C<_get_ts_string($data{completed}//Time::Piece-E<gt>new)> runs before
C<$data{completed}> is converted from a string to a Time::Piece object later
in C<new()>.  Works for UTC strings via the regex fallback, but is fragile
for non-standard formats.

=item m#5: Instance-method guard in _get_ts_string is dead code (in _get_ts_string())
...
=item m#6: encode_json fallback unreachable (in _collect_secrets_from_paths())
...
=item C#2: Dead unreachable timestamp fallback (in new())

The C<unless (defined($timestamp))> block cannot fire: C<$timestamp> is
already defined by the C<//= _get_ts_string(...)> line above it, and
C<_get_ts_string> either returns a string or bails. Categorised as Critical
when first found, but it has no runtime impact -- hence Info.

=item I#1: Mutable package globals unprotected (package scope)
