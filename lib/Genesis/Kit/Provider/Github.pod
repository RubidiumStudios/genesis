...
The C<Service::Github> object that performs the actual API calls.

Everything below delegates to it. The provider holds configuration and
kit-shaped conventions; the remote holds the HTTP.

B<Parameters:> None.

B<Returns:> The C<Service::Github> object.

B<Examples:>

  my $remote = $provider->remote();
  # returns the Service::Github object

=head2 domain

  my $domain = $provider->domain();

The Github domain the provider talks to.

B<Parameters:> None.

B<Returns:> The domain string.
...
=head2 organization
  ...
  my $org = $provider->organization();

The Github organization kits are fetched from.

B<Parameters:> None.

B<Returns:> The organization name string.

B<Examples:>

  print $provider->organization();
  # prints 'my-org'

=head2 credentials

  my $creds = $provider->credentials();

The credentials used for authenticated API requests.

Sourced by C<Service::Github> from C<GITHUB_USER> and C<GITHUB_AUTH_TOKEN>,
not from provider configuration -- so credentials never land in a repo's
C<.genesis/config>.

B<Parameters:> None.

B<Returns:> The credentials value, or C<undef> when none are configured.
Unauthenticated access still works, at Github's lower rate limit.

B<Examples:>

  my $creds = $provider->credentials();
  # returns undef when GITHUB_AUTH_TOKEN is unset

=head2 tls

  my $tls = $provider->tls();

The TLS mode for API requests.

B<Parameters:> None.

B<Returns:> One of C<'yes'>, C<'no'>, or C<'skip'>.
...
=head2 check
  ...
  my $err          = $provider->check($url);
  my ($code, $err) = $provider->check($url);
...
B<(optional)> The URL to test. Defaults to the organization's repository
listing.

=back

B<Returns:> In list context, C<($http_code, $error_message)>. In scalar
context, the error message, or C<undef> on success -- so a scalar-context
call reads as false when everything is well.

B<Examples:>

  my $err = $provider->check();
  # returns undef when the provider is reachable

  my ($code, $err) = $provider->check();
  # returns (404, '...') for an organization that does not exist

=head2 repos_url

  my $url = $provider->repos_url();
  my $url = $provider->repos_url($page);
...
B<(optional)> Page number, for paginated results.
...
=head2 releases_url
  ...
  my $url = $provider->releases_url($name);
  my $url = $provider->releases_url($name, $page);

The API URL listing releases for a kit.
...
The kit name B<without> the C<-genesis-kit> suffix -- the provider appends
it, since that suffix is this provider's naming convention rather than part
of the kit's identity.

=item C<$page>

B<(optional)> Page number.
...
B<Examples:>
  ...
  print $provider->releases_url('bosh');
  # prints the releases URL for bosh-genesis-kit

=head2 base_url

  my $url = $provider->base_url();

The base API URL for the configured domain.

B<Parameters:> None.

B<Returns:> The base URL string.

B<Examples:>

  print $provider->base_url();
  # prints 'https://api.github.com'

=head2 config

  my %cfg = $provider->config();

The provider's configuration, in the form C<new> accepts -- so a provider can
be serialised into a repo's config and reconstructed later.

B<Only non-default values are included>, beyond the always-present C<type>
and C<organization>. A config that recorded every default would pin them,
and a later change to what Genesis defaults to would not reach repositories
already configured.

B<Parameters:> None.

B<Returns:> A hash with C<type> and C<organization> always, plus C<label>,
C<domain> and C<tls> when they differ from their defaults.

B<Examples:>

  my %cfg = $provider->config();
  # returns ( type => 'github', organization => 'my-org' )

  # With a custom label and domain:
  my %cfg = $provider->config();
  # returns those two plus label and domain
