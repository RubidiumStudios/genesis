...
=head2 opts_slot
  ...
  my $slot = Genesis::Kit::Provider->opts_slot();

The key under which this handler's parsed options are stored in the global
command-options hash.

Subclasses inherit the base value unless they need their options kept
separate. Because the slot names a shared structure, two providers returning
the same value would overwrite each other's parsed options.

B<Parameters:> None.

B<Returns:> The slot name string. The base class returns
C<'kit_provider'>.

B<Examples:>

  my $slot = Genesis::Kit::Provider->opts_slot();
  # returns 'kit_provider'
