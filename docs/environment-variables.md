...
| Variable | Description | Default/Values | Set By | Used By | Notes |
|----------|-------------|----------------|--------|---------|-------|
| `GENESIS_CALLER_DIR` | Directory of the caller for Genesis operations. Represents the original directory where Genesis was invoked. | - | bin/genesis main() | Genesis::Env (path resolution)<br />bin/genesis (context maintenance) | - |
...
| `GENESIS_ORIGINATING_DIR` | Represents the directory from which Genesis was invoked. | - | Genesis::Env | /lib/Genesis/Commands/Bosh.pm (sets working dir for BOSH)<br />/lib/Genesis/Commands/Info.pm (adjusts working dir)<br />/lib/Genesis/Env.pm (tracks dir for env ops) | - |
