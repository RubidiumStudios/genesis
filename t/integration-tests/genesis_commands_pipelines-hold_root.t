...
subtest 'the no-argument hold covers the root and stops there' => sub {
	# Five rows, the first of which is the restoration this row asserts in
	# its own words, which is why the run is given restore => 0.
	#
	# The two reason rows are what discriminate: a hold whose loop ran over
	# nothing exits 0 and says nothing, and both of them go red.  The fifth
	# row is a guard over the path composition T320 owns, and it holds
...
subtest 'the no-argument release clears the root and stops there' => sub {
	...
	#
	# The two no_secret rows are what discriminate: a release whose loop ran
	# over nothing leaves both records standing and both go red.  The last
	# row is the guard T320's path composition carries, as in the subtest
	# above.
	plan tests => 7;
...
subtest 'a root that declares no pipeline environments refuses' => sub {
	...
	#
	# All four rows discriminate: without the refusal each command's loop
	# runs over an empty list, exits 0, and prints nothing, which is the
	# silent success this subtest exists to keep out.
	plan tests => 6;
	...
	like(unfolded($out, $err), qr/No environments with pipeline metadata/i,
		'and says that the root declares none rather than holding nothing quietly');
...
subtest "the second root's own no-argument forms stop at that root" => sub {
	...
	#
	# The two rows that read the bosh reason are what discriminate the
	# stopping; the vault rows discriminate the covering, and go red for a
	# loop that ran over nothing the way the rows above them do.
	plan tests => 7;
