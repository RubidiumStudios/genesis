...
subtest 'PIPELINE command in a repo with legacy ci.yml -> bail' => sub {
	...
	like($err // '', qr/Write\s+them\s+into\s+the\s+pipeline:\s+block/,
		'bail message says where the values go');
	like($err // '', qr/\.genesis\/config/,
		'and names the file that declares them');
};
