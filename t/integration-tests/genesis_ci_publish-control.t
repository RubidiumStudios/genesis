...
subtest 'a control that moved during the walk refuses the publish' => sub {
	...
	# This is the temporary status rather than the data one, because another
	# clone moved control and nothing the operator wrote is wrong.  That a
	# re-run then succeeds is T339's row, not this one.
