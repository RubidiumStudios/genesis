...
#
# The third row settles whether the refusal on an illegal initial state
# applies to a command that resolves nothing.  A hand commit the operator's
# own clone has not published is a legal state and the one drift_for exists
# to report, so a report that refused it would refuse to explain the one
# thing it was written to explain.
use strict;
use warnings;
use utf8;
