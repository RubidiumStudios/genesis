...
subtest 'authenticate() - already authenticated returns self' => sub {
	...
	# during the new renewer-arming path) are explicitly excluded,
	# because the invariant we care about is "no credential-based safe
	# auth attempt happens when we're already authenticated."
	my @auth_queries;
	no warnings 'redefine';
