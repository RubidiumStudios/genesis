...
subtest "the label is the provider's alone" => sub {
	...
	# parsed configuration in others, and a branch built at either one
	# would take the label for its name, when a branch is named for the
	# environment and its deployment type and never for the provider's
	# label.  A branch is named by the word, by a ref path, by the two
	# porcelain forms that create one without saying branch at all, and
	# by the HEAD refspec a push composes.  The two porcelain forms allow
	# punctuation between the verb and its flag, because git is called
	# here as a list of arguments far more often than as a command line.
	my @pms = sort split /\n/, qx{find lib -name '*.pm'};
	cmp_ok scalar(@pms), '>', 0,
